uint8_t Query_IDshare_R[] = {
  0x01, 
  0x10,
  0x09, 0x80,
  0x00, 0x06,
  0x0C, 
  0x00, 0x00, 0x00, 0x0F,
  0x00, 0x00, 0x00, 0x02,
  0x00, 0x00, 0x00, 0x01,
  0x00, 0x00};

uint8_t Query_IDshare_L[] = {
  0x02, 
  0x10,
  0x09, 0x80,
  0x00, 0x06,
  0x0C, 
  0x00, 0x00, 0x00, 0x0F,
  0x00, 0x00, 0x00, 0x02,
  0x00, 0x00, 0x00, 0x02,
  0x00, 0x00};

uint8_t Query_READ_R[] = {
  0x01, 
  0x10, 
  0x09, 0x90,
  0x00, 0x06,
  0x0C,
  0x00, 0x00, 0x00, 0x40,
  0x00, 0x00, 0x00, 0x7C,
  0x00, 0x00, 0x00, 0x7D,
  0x00, 0x00};

uint8_t Query_READ_L[] = {
  0x02, 
  0x10, 
  0x09, 0x90,
  0x00, 0x06,
  0x0C,
  0x00, 0x00, 0x00, 0x40,
  0x00, 0x00, 0x00, 0x7C,
  0x00, 0x00, 0x00, 0x7D,
  0x00, 0x00};

uint8_t Query_WRITE_R[] = {
  0x01, 
  0x10, 
  0x09, 0xA8,
  0x00, 0x0C,   
  0x18,
  0x00, 0x00, 0x00, 0x2D,   // 0x09A8-0x09A9 DDO drive mode
  0x00, 0x00, 0x00, 0x2E,   // 0x09AA-0x09AB DDO position
  0x00, 0x00, 0x00, 0x2F,   // 0x09AC-0x09AD DDO speed
  0x00, 0x00, 0x00, 0x30,   // 0x09AE-0x09AF DDO Accel rate
  0x00, 0x00, 0x00, 0x31,   // 0x09B0-0x09B1 DDO Decel rate
  0x00, 0x00, 0x00, 0x33,   // 0x09B2-0x09B3 DDO Trigger
  0x00, 0x00};

uint8_t Query_WRITE_L[] = {
  0x02, 
  0x10, 
  0x09, 0xA8,
  0x00, 0x0C,   
  0x18,
  0x00, 0x00, 0x00, 0x2D,   // 0x09A8-0x09A9 DDO drive mode
  0x00, 0x00, 0x00, 0x2E,   // 0x09AA-0x09AB DDO position
  0x00, 0x00, 0x00, 0x2F,   // 0x09AC-0x09AD DDO speed
  0x00, 0x00, 0x00, 0x30,   // 0x09AE-0x09AF DDO Accel rate
  0x00, 0x00, 0x00, 0x31,   // 0x09B0-0x09B1 DDO Decel rate
  0x00, 0x00, 0x00, 0x33,   // 0x09B2-0x09B3 DDO Trigger
  0x00, 0x00};

uint8_t Query_NET_ID_READ[] = {
  0x0F, 
  0x03, 
  0x00, 0x00,
  0x00, 0x0E,
  0x00, 0x00};

uint8_t Query_NET_ID_WRITE[] = {
  0x0F, 
  0x10,
  0x00, 0x00,
  0x00, 0x18,
  0x30,
  0x00, 0x00, 0x00, 0x10,   // 01:drive mode
  0x00, 0x00, 0x00, 0x00,   //    position
  0x00, 0x00, 0x05, 0xE8,   //    speed
  0x00, 0x00, 0x03, 0xE8,   //    Accell rate
  0x00, 0x00, 0x03, 0xE8,   //    Decell rate
  0x00, 0x00, 0x00, 0x01,   //    Trigger
  0x00, 0x00, 0x00, 0x10,   // 02:drive mode
  0x00, 0x00, 0x00, 0x00,   //    position
  0x00, 0x00, 0x05, 0xE8,   //    speed
  0x00, 0x00, 0x03, 0xE8,   //    Accell rate
  0x00, 0x00, 0x03, 0xE8,   //    Decell rate
  0x00, 0x00, 0x00, 0x01,   //    Trigger
  0x00, 0x00};

uint8_t Query_RIN_START_ON_R[] = {
  0x01, 0x10,
  0x00, 0x5A,
  0x00, 0x0E,
  0x1C,
  0x00, 0x00, 0x00, 0x30,
  0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x05, 0xE8,
  0x00, 0x00, 0x03, 0xE8,
  0x00, 0x00, 0x03, 0xE8,
  0x00, 0x00, 0x03, 0xE8,
  0x00, 0x00, 0x00, 0x01,
  0x00, 0x00};

uint8_t Query_RIN_START_ON_L[] = {
  0x02, 0x10,
  0x00, 0x5A,
  0x00, 0x0E,
  0x1C,
  0x00, 0x00, 0x00, 0x30,
  0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x05, 0xE8,
  0x00, 0x00, 0x03, 0xE8,
  0x00, 0x00, 0x03, 0xE8,
  0x00, 0x00, 0x03, 0xE8,
  0x00, 0x00, 0x00, 0x01,
  0x00, 0x00};

uint8_t Query_Read_Temperature_R[] = {
  0x01, 0x03,                     // ID=1
  0x00, 0xF8,
  0x00, 0x04,
  0x00, 0x00};

uint8_t Query_Read_Temperature_L[] = {
  0x02, 0x03,                     // ID=2
  0x00, 0xF8,
  0x00, 0x04,
  0x00, 0x00};

uint8_t Query_Write_Son_R[] = {
  0x01, 0x10,                     // ID=1
  0x00, 0x7C,
  0x00, 0x02,
  0x04,
  0x00, 0x00, 0x00, 0x01,         // turn on 
  0x00, 0x00};

uint8_t Query_Write_Son_L[] = {
  0x02, 0x10,                     // ID=2
  0x00, 0x7C,
  0x00, 0x02,
  0x04,
  0x00, 0x00, 0x00, 0x01,         // turn on 
  0x00, 0x00};

uint8_t Query_Write_Soff_R[] = {
  0x01, 0x10,                     // ID=1
  0x00, 0x7C,
  0x00, 0x02,
  0x04,
  0x00, 0x00, 0x00, 0x00,         // turn off
  0x00, 0x00};

uint8_t Query_Write_Soff_L[] = {
  0x02, 0x10,                     // ID=2
  0x00, 0x7C,
  0x00, 0x02,
  0x04,
  0x00, 0x00, 0x00, 0x00,         // turn off
  0x00, 0x00};

uint8_t Query_Write_Rev_R[] = {
  0x01, 0x10,                     // ID=1
  0x03, 0x48,
  0x00, 0x02,
  0x04,
  0x00, 0x00, 0x00, 0x01,         // reverse
  0x00, 0x00};

