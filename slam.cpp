...
double match_count(std::vector<std::vector<int>> &gmap,
                   std::vector<Point> &pt,
  ...
  for (int ind = 0; ind < pt.size(); ind += 1) {
    ...
    if (2 <= ix && ix < width-2 && 2 <= iy && iy < height-2) {
      /*
            if (gmap[iy][ix] == 1) {
                eval += 1;
            } else if (gmap[iy-1][ix-1] == 1 || gmap[iy-1][ix] == 1 || gmap[iy-1][ix+1]==1 || 
                       gmap[iy][ix-1]==1 || gmap[iy][ix+1]==1 ||
                       gmap[iy+1][ix-1] == 1 || gmap[iy+1][ix] == 1 || gmap[iy+1][ix+1]==1) {
                          eval += 0.8; 
            }
      */
      for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
          if (gmap[iy + dy][ix + dx] == 1)
            eval += 1/(1+abs(dx) + abs(dy));
        }
      }
    }
  }
...
std::tuple<double, double, double, double> optimize_de(
  ...
  for (int generation = 0; generation < generations; generation++) {
    ...
    for (int i = 0; i < population_size; i++) {
      ...
      double eval = match_count(gmap, pt, trial_x, trial_y, trial_a, csize, originX, originY, width, height);
...
int main (int argc, char *argv[]) {
  ...
  double Wxy = 0.6;      // 探索範囲[m]
  double Wa  = M_PI/3;    // 角度[rad]

  const std::string STORE_ROOT_DIR_NAME = "slam_result_251006-1";
  // ディレクトリが存在しない場合は作成
  ...
  const std::string PATH_TO_URGLOG = "./2025/10/05/184420/urglog_b";
  try {
    // ファイルコピー
    fs::copy_file(PATH_TO_URGLOG, STORE_ROOT_DIR_NAME + "/urglog_b", fs::copy_options::overwrite_existing);
    std::cout << PATH_TO_URGLOG + " コピー成功\n";
  } catch (const fs::filesystem_error& e) {
    std::cerr << PATH_TO_URGLOG + " ファイルコピーに失敗しました: " << e.what() << '\n';
  }
